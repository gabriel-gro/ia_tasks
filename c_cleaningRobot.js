...
function makeDiagram(selector) {
    ...
    for (let floorNumber = 0; floorNumber < world.floors.length; floorNumber++) {
        let meuY = 225;
        if(floorNumber >= world.floors.length/2){
            meuY = 450;
        }
...
function returnMyY(location){
    let myY = 100;
    if (location >= 4/2){
        myY = 330;
    }
    console.log(location, myY);
    return myY;
}

function renderWorld(diagram) {
    console.log(diagram.world.floors.length);
    ...
    diagram.robot.style('transform', `translate(${diagram.xPosition((diagram.world.location)%2)}px,${returnMyY(diagram.world.location)}px)`);
}
